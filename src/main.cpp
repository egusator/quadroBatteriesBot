...
string token, startMessage, commandsList, conninfo, currentBatteryID,
...
int main(int argc, char *argv[]) {
	...
	bot.getEvents().onCommand("my_flights",
			[&bot](TgBot::Message::Ptr message) {
				...
				pqxx::result bat =
						tx.exec(
								"select id, special_bat_id from battery where user_id = "
										+ to_string(message->chat->id));
				int i = 0;
				for (auto row : bat) {
					pqxx::result flight = tx.exec(
							"select * from flight where battery_id = "
									+ string(row[0].c_str()));
					for (auto flightRow : flight) {
						messageToSend += to_string(++i) + ") "
								+ string(row[1].c_str()) + " - ";
						pqxx::result type = tx.exec(
								"select name from flight_type where id = "
										+ string(flightRow[2].c_str()));
						messageToSend += string(type[0][0].c_str()) + ", ";
...
/*
	bot.getEvents().onCommand("remove_battery",
				[&bot](TgBot::Message::Ptr message) {
		string messageToSend = "Select a battery to delete from your list(send id, for example: G1): \n";
						pqxx::work tx(*C);
						pqxx::result batteryOfUser =
								tx.exec(
										"SELECT battery_type_id, special_bat_id \
						...
						bot.getApi().sendMessage(message->chat->id, messageToSend);
						waitingForBatteryDeleting = true;
				});
*/
	...
	+ to_string(message->text));
			...
			pqxx::work tx(*C);
			tx.exec(
					"insert into flight(battery_id, flight_type_id, spent_energy, flight_time) values("
							+ currentBatteryID + ", " + currentFlightType + ", "
							+ currentSpentEnergy + "," + spentTime + ")");
			tx.commit();

			bot.getApi().sendMessage(message->chat->id,
					"Added this flight to your flights(check it - /my_flights )\n");
		} /*else if (waitingForBatteryDeleting){}*/
